...
from new_ddc.new_sym_net import SymNet
...
def load_sp_model(sp_ckpt_fp, sess, batch_size=128):
    with tf.compat.v1.variable_scope('model_sp'):
        model_sp = OnsetNet(
            ...
            zack_hack=0)
    filtered_vars = filter(lambda v: 'model_sp' in v.name, tf.compat.v1.global_variables())
    model_sp_vars = list()
...
def load_ss_model(ss_ckpt_fp, sess):
    ...
    filtered_vars = filter(lambda v: 'model_ss' in v.name, tf.compat.v1.global_variables())
    model_ss_vars = list()
    for v in filtered_vars:
